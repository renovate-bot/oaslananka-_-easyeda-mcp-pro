...
async function main() {
  ...
  let bridgeConnected = false, bridgeVersion = '?', bridgeStatusRaw = '';
  ...
  // 2. RUNTIME METHODS
  console.log('\n── [2/7] Runtime Methods ──\n');
  const required = ['schematic.createNetFlag', 'schematic.createNetPort', 'schematic.connectPinToNet',
    'schematic.connectPinsByNet', 'schematic.validateNetlist', 'project.save'];
  ...
  for (const kw of ['resistor', 'R_0603', 'R_0805', 'capacitor']) {
    const { text: dt } = await toolCall('easyeda_schematic_search_device', { keyword: kw, limit: 10 });
    const d = JSON.parse(dt);
    devices = d?.devices || d?.results || [];
    if (devices.length >= 2) break;
  }
  const d0 = devices[0], d1 = devices[1];
  if (!d0 || !d1) { fail_('Search devices', `need 2, got ${devices.length}`); process.exit(1); }
  ...
  const { text: pr1 } = await toolCall('easyeda_schematic_place_component', {
    projectId, deviceUUID: d0id, x: 100, y: 200, rotation: 0, confirmWrite: true });
  ok('Place R1', pr1.slice(0, 150));
  try { r1pid = JSON.parse(pr1)?.primitiveId || ''; } catch {}

  const { text: pr2 } = await toolCall('easyeda_schematic_place_component', {
    projectId, deviceUUID: d1id, x: 500, y: 200, rotation: 0, confirmWrite: true });
  ok('Place R2', pr2.slice(0, 150));
  try { r2pid = JSON.parse(pr2)?.primitiveId || ''; } catch {}
  ...
  if (!r1pid || !r2pid) {
    ...
    for (const c of comps) {
      ...
      if (ref.startsWith('E2E_') || ref === 'R1' || ref === 'R2') {
        ...
        if (!r1pid) r1pid = pid; else if (!r2pid && pid !== r1pid) r2pid = pid;
      }
    }
  }
  ...
  if (testNets.length > 0) ok('validateNetlist → TEST_NET', `refs=${JSON.stringify(testNets[0].refs)}`);
  ...
  const { text: e2 } = await toolCall('easyeda_schematic_connect_pin_to_net', {
    projectId, primitiveId: 'NONEXISTENT_999', pinNumber: '1', netName: 'TEST_NET' });
