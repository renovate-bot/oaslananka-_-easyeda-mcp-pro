...
const server = spawn('node', ['dist/index.js'], {
  ...
  env: { ...process.env, TRANSPORT: 'stdio', LOG_LEVEL: 'silent', TOOL_PROFILE: 'full',
    BRIDGE_RECONNECT_MAX_ATTEMPTS: '0', BRIDGE_RECONNECT_INTERVAL_MS: '1000', BRIDGE_TIMEOUT_MS: '30000' },
});
...
rl.on('line', line => {
  ...
  if (p.id && pending.has(p.id)) {
    ...
    pending.delete(p.id); clearTimeout(timer);
...
async function main() {
  ...
  if (exited) { console.log('❌ Server exited'); process.exit(1); }

  await mcpCall('initialize', { protocolVersion: '2025-11-25', capabilities: {}, clientInfo: { name: 'diag', version: '1' } });
  ...
  for (let i = 0; i < 120; i++) {
    try {
      ...
      bs = text;
      if (JSON.parse(text).connected) { console.log(`✅ Bridge connected (${i + 1}s)`); break; }
    } catch {}
  ...
  const required = ['schematic.createNetFlag','schematic.createNetPort','schematic.connectPinToNet','schematic.connectPinsByNet','schematic.validateNetlist','project.save'];
  console.log('\n=== REQUIRED METHODS ===');
  ...
  const netTools = toolNames.filter(n => n.includes('net') || n.includes('flag') || n.includes('connect') || n.includes('save') || n.includes('validate'));
